/*************************************************************************
	...
	> Created Time: 2016年03月07日 星期六 13时03分51秒
 ************************************************************************/

#include<iostream>
#include"../include/bi_pw_lstm.h"
#include"../include/lstm_tool.h"

using namespace std;

//default initialization
BiPointwiseLSTM::BiPointwiseLSTM()
{
    return;
}

//initialization with model stored in file
BiPointwiseLSTM::BiPointwiseLSTM(string model_file)
{
...
BiPointwiseLSTM::BiPointwiseLSTM(int input_cell_num, int hidden_cell_num, int state_cell_num, int output_num) : 
...
void BiPointwiseLSTM::forwardPass(MatrixXd input_data)
{
    /*
...
void BiPointwiseLSTM::calculateSoftMax()
{
    /*
...
void BiPointwiseLSTM::backwardPass(MatrixXd label)
{
    /*
...
//SGD with a momentum
void BiPointwiseLSTM::stochasticGradientDescent(vector<MatrixXd*> input_datas, vector<MatrixXd*> input_labels, double learning_rate, double momentum_rate)
{
    /*
    ...
    MatrixXd forward_state_forget_gate_weights_derivative = MatrixXd::Zero(forward_lstm.getStateCellNum(), forward_lstm.getStateCellNum() + 1);
    MatrixXd forward_hidden_forget_gate_weights_derivative = MatrixXd::Zero(forward_lstm.getStateCellNum(), forward_lstm.getHiddenCellNum() + 1);
    MatrixXd forward_input_forget_gate_weights_derivative = MatrixXd::Zero(forward_lstm.getStateCellNum(), forward_lstm.getInputCellNum() + 1);
    ...
    MatrixXd backward_state_forget_gate_weights_derivative = MatrixXd::Zero(backward_lstm.getStateCellNum(), backward_lstm.getStateCellNum() + 1);
    MatrixXd backward_hidden_forget_gate_weights_derivative = MatrixXd::Zero(backward_lstm.getStateCellNum(), backward_lstm.getHiddenCellNum() + 1);
    MatrixXd backward_input_forget_gate_weights_derivative = MatrixXd::Zero(backward_lstm.getStateCellNum(), backward_lstm.getInputCellNum() + 1);
...
vector<MatrixXd*> BiPointwiseLSTM::predict(vector<MatrixXd*> input_datas)
{
    /*
...
void BiPointwiseLSTM::saveModel(string file_name)
{
    /*
...
void BiPointwiseLSTM::loadModel(string file_name)
{
    /*
